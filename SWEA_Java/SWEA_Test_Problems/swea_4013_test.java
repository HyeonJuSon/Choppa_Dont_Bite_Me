...
public class swea_4013_test {
	...
	public static void main(String[] args) throws Exception  {
		System.setIn(new FileInputStream("res/swea_4013_test.txt"));
			...
			while(!orders.isEmpty()) { // 모든 회전명령을 수행할때 까지 시행한다.
				...
				if(dir == 1) {//시계방향
					...
					case 1: // 2번 회전시 1번과 3번은 무조건
						if(gears[0][right] != gears[1][left]) rotGear.offer(new int[] {0,-1});
						if(gears[1][right] != gears[2][left]) { // 3번이 움직여지는지 확인
							rotGear.offer(new int[] {2,-1});
							if(gears[2][right]!=gears[3][left]) rotGear.offer(new int[] {3,1});
						}
						break;
					case 2: // 3번 회전시 2번과 4번은 무조건
						if(gears[3][left] != gears[2][right]) rotGear.offer(new int[] {3,-1});
